...
from aiostomp.adapter import MulticastConnection
import aiostomp.colours
from aiostomp.connect import StompConnection10, StompConnection11, StompConnection12
from aiostomp import ConnectionListener, StatsListener

sys.path.append(".")
import aiostomp as stomp
...
class StompCLI(Cmd, ConnectionListener):
    ...
    def __error(self, msg, end="\n"):
        self.stdout.write(aiostomp.colours.BOLD + aiostomp.colours.RED + str(msg) + aiostomp.colours.NO_COLOUR + end)
    ...
    def help(self, usage, description, required=(), optional=()):
        ...
        m = {
            "hl": aiostomp.colours.BOLD + aiostomp.colours.GREEN,
            "nc": aiostomp.colours.NO_COLOUR,
            "usage": usage,
...
\tauto.''', ["destination - the name to subscribe to"], ["ack - how to handle acknowledgements for a message; either automatically (auto) or manually (client)"])
    ...
    def do_sendrec(self, args):
        args = args.split()
        receipt_id = aiostomp.utils.get_uuid()
        if len(args) < 2:
    ...
    def do_version(self, args):
        self.__sysout("%s%s [Protocol version %s]%s" %
                      (aiostomp.colours.BOLD, version_string, self.conn.version, aiostomp.colours.NO_COLOUR))
    do_ver = do_version
